...
Both broke us: the first is absorbed as the `gtk-application-argv` tag
below, the second is resolved by configuration — the examples share the
root-generated store via `codegen: false` (see "Resolved by configuration"
below); upstream ask #10 tracks the underlying type defect. The full
diagnosis for each is in the `gtkx-1-0-migration` epic's `001-notes.md`.
Neither lives in `src/gtkx/bridge/`:
