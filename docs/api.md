...
`gtkx.config.ts`'s `libraries` need not include `"Adw-1"`:
...
- `createSidebarNavigator` — the desktop drawer equivalent on
  ...
  "Two ways to react to size"); the property flip happens inside GTK's own
...
[Architecture: Overview](architecture/overview.md#react-native-gtkxcommon-what-has-no-upstream-counterpart)).
