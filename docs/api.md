...
| `Image`             | GtkPicture                                            | `source={{uri}}`/string — local paths, file:// and **http(s)** (Node fetch → disk cache keyed by URL, in-flight de-duplication), `resizeMode` cover/contain/stretch/center, `onLoad`/`onError`; **`.svg` files load like any other image** — `Gdk.Texture.newFromFilename` rasterizes them via librsvg, no extra code needed (for building vector graphics from state instead of a file, see `Svg` below)                                                                                                                                                                                                                                | no synchronous size from remote images (style sets the size, as in RN); cache is not size-limited yet                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `Svg`               | RnGtkxSvgNode (Gtk.Widget, `Gsk.Path`/`Gtk.Snapshot`) | vector graphics built from state/animation, not a rasterized file (that is `Image`, above) — see the "Svg" section below for the full component set (`Path`/`Rect`/`Circle`/`Ellipse`/`Line`/`Polygon`/`Polyline`/`G`/`Defs`/`LinearGradient`/`RadialGradient`/`Stop`)                                                                                                                                                                                                                                                                                                                                                                   | letterboxes/crops to `viewBox`+`preserveAspectRatio` like real SVG; always clips to its own bounds (no `overflow: visible` opt-out)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
```tsx
import { Circle, G, Path, Rect, Svg } from "react-native"
...
- **Animated**: the numeric props above (shape geometry, `opacity`,
  ...
  `Animated.View` uses for `translate` (`setStoredOffset` +
