...
(`height: withTiming(320)`), that render is produced for you the moment the
animation reaches its target, because otherwise there would not be one. Once
per animation, never once per frame: in a full run of
`spike/core-exports`'s bottom sheet, 176 animation frames on refused
properties produced 4 renders.
...
| `useAnimatedStyle`                                                                                               | Full for `opacity`, `transform`, colours, the insets of an absolutely positioned node and a `width`/`height` whose change is confined to that node — see the boundary above. An animation RETURNED from the updater (`() => ({ opacity: withTiming(1) })`) runs here as it does upstream, on the platform's one frame scheduler; a key animating for the first time is seeded at its target rather than animated to it, and a later run producing the same animation does not restart it. A style whose _shape_ changes between runs costs exactly one React render and rebinds; a running animation costs none, except for the single render that publishes a REFUSED property when its animation settles.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
