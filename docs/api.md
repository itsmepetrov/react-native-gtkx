...
| everything else                                                                     | **Throws**, naming the symbol. `PanGestureHandler` and the legacy handler components, the `Legacy*` aliases, `RefreshControl`, the remaining `use*Gesture` hooks, and the BUTTON family — `RawButton`, `BaseButton`, `RectButton`, `BorderlessButton`, plus `TouchableNativeFeedback` and the deprecated `Touchable` mixin. That boundary is upstream's own: the buttons are not RN components with a handler attached, they are RNGH's own native button views, with a platform ripple and an `activeOpacity` that belong to a widget this platform does not have.                                                                                                                                                               |
...
### What still stops the three libraries this surface was measured against

**Measured by building them, not by reading them** — a probe app under the real
`gtkx build`, with the real published packages and the presets' aliases in
place. That corrects the entry below it twice over, and both corrections are
the same shape: the thing that stops these libraries is not in this surface.

- **`react-native-draggable-flatlist` 4.0.3 — stops at BUILD, on
  `react-native` rather than on `react-native-gesture-handler`.** The two
  missing exports are `findNodeHandle` and `LogBox`
  (`src/components/CellRendererComponent.tsx`,
  `src/components/NestableDraggableFlatList.tsx`). Its RNGH imports all
  resolve. **The earlier claim that RNGH's `FlatList`/`ScrollView` re-exports
  stopped it at import was wrong**, and the mechanism says why: it hands them
  to `Animated.createAnimatedComponent()` at module scope, which reads only
  `displayName` and `name` — both on the stand-in's introspection allowlist,
  so a stand-in binds and constructs without complaint and would have failed at
  first RENDER. The re-exports were still needed; they were not the wall.
  Past those two, the next wall is `useAnimatedScrollHandler`
  (`DraggableFlatList.tsx`, `NestableScrollContainer.tsx`), which
  [`react-native-gtkx/reanimated`](#react-native-reanimated-react-native-gtkxreanimated)
  refuses.
- **`@gorhom/bottom-sheet` 5.2.14 — stops at BUILD, also on `react-native`.**
  `findNodeHandle`, `LogBox`, `Keyboard` and `VirtualizedList`
...
- **`react-native-reanimated-dnd` 2.0.0 — never loads, by design.** Both
  ...
  its API on GTK's own drag-and-drop. Its `Sortable`/`SortableGrid` do take
  RNGH's `FlatList`/`ScrollView` at module scope, so the re-exports are what an
  UNALIASED build would need — but an app never takes that path, and the alias
  is documented as load-bearing rather than convenient.
...
`console.log` do first, so the message that surfaces is the precise one — and
the `createAnimatedComponent` case above is the honest limit of that: a
stand-in that is only ever introspected cannot fail until it is used.
