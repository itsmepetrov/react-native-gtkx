...
| **Row hover**                                                                                                                                         | `Pressable`'s `style={({ hovered }) => …}`. Already works, and works well: `tests/gtk/components/pressable-hover.gtk.test.tsx` asserts that a real `EventControllerMotion` `enter` swaps the widget's CSS class **without a React render at all** (the fast path in `components/pressable.tsx`). `hovered` is a documented platform extension to RN's state callback; `examples/hn-app` and the gallery already use it. |
