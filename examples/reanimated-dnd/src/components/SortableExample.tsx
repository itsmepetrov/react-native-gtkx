...
export function SortableExample({ onBack }: SortableExampleProps = {}) {
  ...
  // See the callbacks on SortableItem below.
    ...
    (props: SortableRenderItemProps<Item>) => {
      ...
      return (
        <SortableItem
          ...
          // Upstream console.logs each of these. This repo forbids
          // console.log outside scripts, and a desktop app has no Metro
          // terminal to watch anyway — so the same four callbacks, with the
          // same signatures, report into the header instead. Which is
          // strictly better for the thing they demonstrate: you can see
          // onMove fire while the row is still under the cursor.
          onMove={(currentId, from, to) => {
            setLastEvent(`onMove — ${currentId}: ${from} → ${to}`)
          }}
          onDragStart={(currentId, position) => {
            setLastEvent(`onDragStart — ${currentId} at ${position}`)
          }}
          onDrop={(currentId, position, allPositions) => {
            setLastEvent(
              `onDrop — ${currentId} at ${position}` +
                (allPositions
                  ? ` (${Object.keys(allPositions).length} positions)`
                  : ""),
            )
          }}
          onDragging={(currentId, overItemId, yPosition) => {
            setLastEvent(
              `onDragging — ${currentId} over ${overItemId} at ${Math.round(yPosition)}`,
            )
          }}
        >
          <SortableTrackRow
...
const styles = StyleSheet.create({
  ...
  modalTitle: {
    ...
    letterSpacing: -0.3,
    color: "#08090E",
    // Upstream declares `fontFamily: "Syne_700Bold"` TWICE in this rule.
    // Harmless there (the second wins, and they are equal); an error here,
    // because tsc runs over the example. An upstream bug this port found.
    fontFamily: "Syne_700Bold",
    marginBottom: 10,
    textAlign: "center",
  },
