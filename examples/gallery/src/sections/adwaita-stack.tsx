// The proof that react-native-gtkx's Adwaita primitives stand on their own: a
// real Adw.NavigationView stack, with NO react-navigation inside this section.
// The "router" here is a useState holding an array of tags. Swap it for your
// own router, a reducer, a URL — the primitive does not care. (The gallery's
// own sidebar around it IS react-navigation, which makes the point sharper:
// the two coexist in one window without knowing about each other.)
//
...
//
// This section does NOT scroll (see src/index.tsx): the pages scroll
// themselves if they need to, and the back gesture is a drag.
...
const styles = StyleSheet.create({
  ...
  // The page bodies deliberately do NOT set a colour: inside Adwaita chrome
  // the theme's own foreground is already correct, and naming one here would
  // be the hand-picked hue #86 removed everywhere else.
  title: { fontSize: 16, fontWeight: "600" },
  body: { fontSize: 14, color: palette.textDim },
  counter: { fontSize: 13, color: palette.textDim },
  toolbar: { flexDirection: "row", gap: 8, alignItems: "center" },
  gtkButton: { width: 72, backgroundColor: palette.accent, borderRadius: 6 },
})

export const AdwaitaStackSection = () => {
  ...
  return (
    // `NavigationStack` renders a RAW `Adw.NavigationView` — a gtkx binding
    // with no Yoga node of its own, unlike the wrapped `AdwBottomSheet` in
    // "Widget hosting" which takes a `style` directly. As a standalone app
    // this was the window's only child and GTK allocated it; inside a React
    // Native layout root nothing does, and it draws nothing while GTK logs
    // "Trying to snapshot AdwNavigationView without a current allocation".
    // `Widget` is the documented bridge: it puts a measured Yoga leaf around
    // the widget, which is what gives it an allocation.
    <Widget style={styles.stack}>
      <NavigationStack
        ref={viewRef}
        stack={stack}
