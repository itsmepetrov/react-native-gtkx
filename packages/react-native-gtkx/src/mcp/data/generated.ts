...
export const PORTABLE_COMPONENTS = [
  ...
  },
  {
    ...
    supported: "—",
    differences:
      "A desktop window has no notch to inset around, so this renders exactly as `View`, with no safe-area insets applied.",
  },
  {
    name: "StatusBar",
    subpath: "react-native",
...
export const DOC_CHUNKS = [
  ...
  {
    doc: "docs/gtkx-1.0-notes.md",
    heading: "How each was checked against 1.0",
  ...
  {
    doc: "docs/gtkx-1.0-notes.md",
    heading: "1.0-WORKAROUND(use-signal-stale-handler)",
  ...
  {
    doc: "docs/gtkx-1.0-notes.md",
    heading: "1.0-WORKAROUND(runtime-dedupe)",
    text: "Two bundled copies of the gtkx runtime still double-init GLib and abort; nothing guards against it across two DISTINCT `.node` files. Reproduced on 1.0: `g_log_set_writer_func() called multiple times`, SIGABRT — our workaround: `src/vite/index.ts` puts `resolve.dedupe` over `@gtkx/*` + `react` (+ `@react-navigation/*` for its context) into the preset every app inherits — removed when: Idempotent runtime init upstream across distinct native-addon files, or an error that names the duplicate",
  },
  {
    doc: "docs/gtkx-1.0-notes.md",
    heading: "1.0-WORKAROUND(prop-portal)",
    text: '`createPortal(children, container, key?)` is byte-for-byte the rc.3/rc.4 signature and can still only target a container\'s DEFAULT slot ("children"). `@gtkx/react`\'s public entry and its `/internal` subpath both still withhold the `"gtkx:prop"` element — `/internal` now re-exports `createElementComponent` (the function that BUILDS an intrinsic element by GType name) but not the `Prop` literal itself, which stays a sibling export inside `element.js` that no public or `/internal` path reaches — our workaround: `gtkx/bridge/slot-portal.ts` restates the `"gtkx:prop"` element name and wraps it in `createSlotPortal(children, target, slot)` — the one line that would move if gtkx renames it. `WindowActions`/`ApplicationActions`/`WindowControllers` are built on it — removed when: gtkx exports a slot-aware portal (or the prop element) from a public entry point',
  },
  {
    doc: "docs/gtkx-1.0-notes.md",
    heading: "1.0-WORKAROUND(renderhook-no-window)",
    text: "`renderHook` still mounts into a bare `Gtk.Box` — 1.0's `render-hook.js` still passes `container: new Gtk.Box()` unconditionally, and `RenderHookOptions` still carries only `wrapper` and `initialProps` — so window-dependent APIs have no toplevel to read — our workaround: Hook tests create a window with `render()` first (`tests/gtk/apis/dimensions.test.tsx`); packaged for consumers as `renderHookWithWindow` (`react-native-gtkx/testing`) — removed when: `renderHook` mounts into the same harness window `render` uses",
  },
  {
    doc: "docs/gtkx-1.0-notes.md",
    heading: "1.0-WORKAROUND(gtk-application-argv)",
  ...
  {
    doc: "docs/gtkx-1.0-notes.md",
    heading: "1.0-WORKAROUND(dialog-component-ref-widen)",
    text: "The shared `createDialogComponent` factory backing all five generated Adw dialog JSX wrappers (`AboutDialog`/`AlertDialog`/`Dialog`/`PreferencesDialog`/`ShortcutsDialog`) types its returned component's props as `PresentedProps<Adw.Dialog>`, where `Adw.Dialog` resolves against WHATEVER `@gtkx/gi` store `@gtkx/react` itself sees — the workspace root's, in an npm workspace — not each app's own freshly-generated per-app store. Every app whose store differs from the root's fails `gtkx build`/`gtkx codegen` with 5 ref-covariance typecheck errors, one per dialog type — our workaround: `patch-package` patch (`patches/@gtkx+react+1.0.0.patch`, `postinstall: patch-package`) widens the factory's returned component's props to `any` in the `.d.ts` only — `dialog.js`'s runtime implementation is fully duck-typed, so this is a types-only fix — removed when: `@gtkx/react`'s `createDialogComponent` stops hardcoding a single `@gtkx/gi` store's `Dialog` type (generic over the component's own props, or typed loosely enough to survive multiple per-app stores in one workspace)",
  },
  {
